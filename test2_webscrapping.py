...
precioProducto = {}
listaURLs = supermercadoURLs.copy()
...
precios_disponibles = {k: v for k, v in precios_validos.items() if isinstance(v, (int, float))}

if precios_disponibles:
    precio_min = min(precios_disponibles.values())
    supermercado_min = min(precios_disponibles, key=precios_disponibles.get)
    print(f"\n El precio mínimo es {precio_min:.2f} € en {supermercado_min}")
else:
    precio_min = None
    print("\n No se encontró ningún precio válido.")

# Muestra todos los precios recolectados
print("\nResumen de precios encontrados:")
for k, v in precioProducto.items():
    print(f"  {k}: {v}")

# Guarda en variable
print(f"\nVariable 'precio_min' = {precio_min}")
